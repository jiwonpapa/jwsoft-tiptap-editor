...
`npx vite --host 127.0.0.1` 실행 후 `/harness/ui-preview.html`을 열면 메뉴와 밝은/어두운 테마를 확인할 수 있습니다. 현재 편집기 소스를 사용하며 서버 저장·파일 업로드·운영 인증을 수행하지 않습니다. 자동 검증은 `make browser-check`로 UI 5개와 SNS 1개 suite를 모두 실행합니다. 사용자의 실제 브라우저 세션을 점유하지 않습니다.
