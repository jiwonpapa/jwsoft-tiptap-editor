...
- 플러그인 업데이트 직후 관리자 SPA가 이전 언어 사전을 유지해 새 설정명이 번역 키로 노출되던 문제를 설정 화면 진입 시 확장 언어팩 재동기화로 보완. 이미지 편집 설정명과 설명도 대상·기능·원본 보존 여부가 바로 드러나도록 개선
- 선택 요소 인라인 도구의 가상 툴팁이 팝오버 영역을 늘려 우측·하단 스크롤바를 만들던 문제 수정. 이미지 편집을 켠 경우 선택 이미지의 인라인 도구에 `이미지 편집`을 직접 노출하고 크롭 전용 아이콘 대신 조절 아이콘을 적용했으며 도움말에 사용 경로 추가
- G7 병합 플러그인 번들에서 실행될 때 이미지 편집 지연 로드 주소를 병합 번들 옆으로 잘못 계산해 `이미지 편집 도구를 불러오지 못했습니다`가 발생하던 문제 수정. `G7Config.pluginAssets`의 플러그인 전용 URL을 기준으로 확장자 유지·제거 모드 모두 지원
...
- 실제 PHP 요청에서 파일로 파싱되지 않는 multipart PUT을 사용해 MP4 청크의 `chunk`·`checksum` 필수 검증이 실패하던 문제 수정. 청크 전송과 공개 라우트를 multipart POST로 통일

### Added

- 관리자에서 독립적으로 켜고 끌 수 있는 `jw-editor` 내부 이미지 편집 하위 모듈 계약. 업로드 이미지의 자르기·회전·보정 결과를 새 파일로 저장하고 기존 이미지의 설명·배치 속성과 원본 파일을 보존
- MIT Filerobot 이미지 편집기를 고정 버전으로 자체 번들하고, 기능을 열 때만 플러그인 자산에서 지연 로드하는 모달 UX

### Security

- 편집 대상을 플러그인 관리 JPEG·PNG·WebP로 제한하고 결과를 기존 인증 업로드 API에서 MIME·크기·픽셀·권한 재검증. 외부 URL·GIF·AVIF·임의 실행 HTML은 편집 대상에서 제외

### Tests

- MP4 청크 요청의 POST·파일·checksum 계약과 G7 라우트 메서드 회귀 검사 추가

### Maintenance

- 중첩된 이미지 편집기 의존성의 라이선스 파일을 G7 설치기가 제외하지 않는 `_nested` 경로로 패키징하고, 릴리스 ZIP의 모든 `node_modules` 경로를 차단

## [0.1.6] - 2026-09-06

### Fixed

- 빈 커서에서 링크 적용 시 본문이 비어 있던 문제 수정. 표시 문구를 지정하거나 주소 자체를 삽입하며, 기존 선택 글자와 서식을 유지한 링크 수정·해제 제공
- Ctrl/⌘ + Shift + V의 텍스트 붙여넣기는 리치 HTML·Markdown·자동 링크·SNS 변환을 거치지 않도록 분리. 일반 붙여넣기의 허용 서식과 Shift+Insert 동작은 유지

### Added

- 도구 더보기에 ‘텍스트만 붙여넣기’ 모달 추가. 모바일에서도 선택 위치에 여러 줄의 순수 텍스트를 삽입하고 실행취소 가능
- 미저장 본문이 있을 때 브라우저 새로고침·탭 닫기 기본 경고. 그누7의 공개 변경 상태와 저장 완료 상태를 구독하며 저장 중·실패 시 경고 유지, 종료 시 구독 해제
- 늦게 도착한 변경 없음 상태를 저장 성공으로 처리하지 않으며, 저장 요청 중 추가 편집한 본문은 미저장 경고 유지
- G7 상태 구독 API가 없는 커스텀 호스트는 해제 불가능한 미저장 경고를 설치하지 않음

### Security

- 사용자 표시 문구와 순수 텍스트를 HTML로 해석하지 않고 문서 텍스트 노드로 삽입. 인라인 style·임의 class/data-*·이벤트 속성·실행 HTML 저장 금지 및 서버 정제 정책은 유지
- 자동저장·브라우저 본문 보관·DB 변경·그누7 코어 수정 없음. 미저장 경고는 브라우저 지원과 사용자 상호작용에 의존하며 모바일 강제 종료나 모든 SPA 내부 이동을 보장하지 않음

### Maintenance

- GitHub 후보·최종 릴리스 본문에 해당 버전 CHANGELOG를 자동 포함. 누락·빈 내용·중복 버전 항목이면 게시를 거부하고 기존 태그·ZIP 불변 정책 유지
- 링크 삽입·보안 입력·순수 텍스트·미저장 경고의 단위 및 데스크톱/모바일 Chromium 회귀 추가. 기존 62개 출시 기준은 변경하지 않음

## [0.1.5] - 2026-09-05

### Security

- 서버 미리보기 연결은 검증된 공개 IPv4에 고정. 이중 주소 사이트는 유지하며, 주소만으로 구분할 수 없는 IPv6 변환망 우회를 막기 위해 IPv6 전용 사이트의 카드 생성은 거부하고 원문 링크를 유지
- 링크 미리보기 목적지 검증을 PHP 8.2+의 전역 IP 기준으로 강화. 비전역·멀티캐스트·IPv4 변환 IPv6 주소를 거부하고 DNS 응답과 리다이렉트에도 동일하게 적용
- 네트워크 없는 주소 경계·혼합 A/AAAA 회귀 및 실제 미리보기 서비스의 HTTP 이전 차단·연결 IP 고정 회귀 추가. 정상 공개 사이트와 기존 SNS 표시·설정은 유지

## [0.1.4] - 2026-09-05

### Fixed

- npm·Composer·Python advisory 실행과 실패 원문을 Python 실행 증거로 보존하고 기존 출시·배포 gate의 audit 항목에 연결. CI 증거에 전체 명령 로그와 결과를 포함
- 배포 성공 단독 기록기를 폐기하고 Python 트랜잭션에 원격 실행·설치 파일·활성 상태·HTTP smoke·staging 승격 증거를 결합
- 줄 끝/블록 주석의 lint 해제와 루트/다른 경로의 신규 범용 Node 하네스 우회 차단 및 금지 사례 회귀 추가
- 소스/패키지 라이선스 증거를 분리하여 기본 검사에 의한 패키지 증거 덮어쓰기를 방지
- AGENTS의 공식 SNS SDK 예외를 승인된 ADR-0018과 동기화. 제품 기능·저장 정책·62개 출시 기준은 유지
