import { tokenLabels } from "@/editor/toolbarLabels";
import { bindAsyncForm } from "@/editor/asyncEvent";
import { uploadPhaseLabel } from "@/editor/uploadProgress";
...
function createImageDialog(
  ...
  bindAsyncForm(form, apply, error, locale, async (event) => {
    event.preventDefault();
    event.stopPropagation();
    error.hidden = true;
    ...
    if (selected && uploadEnabled) {
      ...
      try {
        const uploaded = await uploadEditorMedia(selected, {
          ...
          onPhase: (phase) => {
            if (request.signal.aborted) return;
            progressLabel.textContent = uploadPhaseLabel(phase, locale);
            if (phase === "starting") {
              progressBar.value = 0;
