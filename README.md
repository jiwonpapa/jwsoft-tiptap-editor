...
- **이미지** — 파일 선택·드래그 앤 드롭·붙여넣기 업로드, 업로드 상태·실패 재시도, 정렬·크기·대체 텍스트·캡션. 선택 기능인 이미지 편집을 켜면 관리 중인 JPEG·PNG·WebP를 자르기·회전·보정·필터·크기 변경 후 새 파일로 저장합니다.
- **동영상** — MP4 청크 업로드와 재개, 원본 파일명·확장자 보존, 실제 영상 비율에 맞춘 반응형 플레이어. YouTube·Vimeo 지원 URL 삽입.
- **SNS와 링크** — X·Facebook·Instagram·TikTok 화이트리스트 게시물을 공식 표시하고, 다른 HTTPS URL은 링크 카드로 처리합니다. 주소와 공식 퍼가기 코드를 모두 받을 수 있습니다.
...
Facebook 도메인의 모든 페이지가 게시물 임베드 대상인 것은 아닙니다. 현재 소스의
지원 형식은 일반 `계정/posts/ID`·`계정/videos/ID`, 사진 `photo/?fbid=`·`photo.php?fbid=`,
고유링크 `permalink.php`·`story.php`, `watch/?v=`, `reel/ID`입니다.
`share/p|v|r/토큰`과 `fb.watch/토큰`은 서버가 원본 허용 게시물 주소를 확인한 경우에만
삽입합니다. 프로필·홈·그룹 목록이나 비공개·삭제·접근 제한 콘텐츠를 강제로 표시하지 않습니다.

X는 현재 `계정/status/ID`·`i/status/ID`·`i/web/status/ID`와 허용된 X/Twitter 호스트만 지원합니다.
사진/영상 상세 경로·`t.co` 등 모든 주소 형식의 지원을 뜻하지 않습니다.
Instagram은 공개 `p/ID`·`reel/ID`·`tv/ID`, TikTok은 공개 `@계정/video/ID`를 지원합니다.
YouTube·Vimeo는 링크 카드가 아닌 반응형 동영상 플레이어로 삽입합니다. 여섯 제공자 외 SNS는
공식 실행 콘텐츠가 아니라 일반 HTTPS 링크 카드로 처리합니다.
이 지원 범위는 소스 기준이며, 설치 서버에는 해당 수정본 업데이트 후 적용됩니다.
실제 성공·미완료 항목과 제공자 제한은 [URL 유형별 검증 기록](docs/acceptance/facebook-url-regression.md)에서 구분합니다.
...
- 링크는 선택 글자에 적용하거나, 빈 커서에서 주소/표시 문구를 바로 삽입합니다.
- 서식 없이 넣으려면 `Ctrl/⌘ + Shift + V` 또는 **도구 더보기 → 텍스트만 붙여넣기**를 사용합니다. 모달에서는 HTML·자동 링크·SNS 변환 없이 여러 줄 텍스트를 넣습니다.
- 이미지·동영상 도구를 열어 파일을 선택하고 업로드 상태를 확인한 뒤 삽입합니다.
- 관리자가 **이미지 편집**을 켠 경우 본문 이미지를 선택한 뒤 툴바의 이미지 편집을 누릅니다. 편집본 저장은 새 파일을 만들며 원본 이미지와 대체 텍스트·제목·캡션·정렬·표시 크기를 유지합니다. GIF·AVIF·외부 이미지는 첫 버전의 편집 대상이 아닙니다.
- 빈 줄에 지원 URL 또는 공식 퍼가기 코드를 붙여넣거나 URL 입력 후 Enter를 누르면 설정에 따라 플레이어 또는 카드로 변환합니다.
- 하단 왼쪽 `?`에서 짧은 도움말과 **HTML 소스 보기·복사**를 엽니다. 소스는 읽기 전용이며 저장된 원본과 다를 수 있습니다.
- 에디터 입력만으로 글이 저장되지는 않습니다. 게시판·상품·페이지의 저장 버튼을 사용하세요.
- 미저장 본문이 있으면 새로고침·탭 닫기 때 브라우저 기본 경고를 요청합니다. 저장 성공 후 해제하며 자동저장/초안 보관은 하지 않습니다. 내부 SPA 이동은 그누7 화면의 취소 확인에 따르며 모든 이동·모바일 강제 종료를 보장하지 않습니다.
...
TypeScript + Tiptap/ProseMirror 편집 엔진, PHP 플러그인·서버 검증으로 구성합니다. 내부 확장은 **기본 편집 / 이미지 / 표 / 미디어 / SNS** 모듈로 조립합니다. 이미지 편집처럼 큰 선택 기능은 내부 feature registry에서 ON일 때만 연결하고 자체 포함 번들을 최초 사용 시 지연 로드합니다. 업로드·미디어 표시·G7 연결·정책 검증은 분리되어 있으며, 개발자가 소스에서 확장한 뒤 하나의 검증된 패키지로 배포합니다.
