...
- 같은 ZIP `7660acd40e5a81413ede8ddaa9da83d31e43012b86acdfcace2580a95009792f`으로 후보 57/57, 실제 GitHub 수명주기 후 60/60, staging 적용 후 61/61, production 적용 후 최종 62/62를 통과했습니다. RC1·RC2 ZIP은 보존했습니다.
- 승인된 공유 로컬 사이트와 온라인 모두 JWSoft RC3 active / CKEditor inactive입니다. 주요 런타임 파일 7개와 공개 합본 JS가 패키지와 일치하며 홈·health·관리자 로그인은 HTTP 200입니다. 온라인 staging/production은 승인된 동일 대상에 각각 실제 적용한 것입니다.
- 공유 로컬의 콘텐츠 15개 테이블과 모듈 목록이 보존됐습니다. 온라인 페이지 6건·게시글 50,011건·상품 20,000건, 총 70,017건의 본문 해시와 설정이 배포 전후 동일합니다. DB·플러그인 백업을 원격과 로컬에 보존했습니다.
- 기능 실증은 전용 실제 G7의 인증 화면에서 수행했습니다. 온라인 게시판 접근은 로그인으로 제한되어 운영 인증 작성/업로드를 재수행한 것으로 보고하지 않습니다. 배포 후 원격 검증은 동일 ZIP·활성 상태·런타임·콘텐츠 보존·HTTP 응답·공개 화면 범위입니다.
- 상세 배포 증거는 로컬 `test-results/deploy/staging.json`, `production.json`, 최종 `test-results/release/stable-readiness.json`에 있으며 서버 주소·자격증명·DB 백업은 커밋하지 않습니다.
