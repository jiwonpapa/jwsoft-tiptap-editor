...
- 최신 공개 버전 뒤 패키지 파일이 하나라도 바뀌면 `package.json`·lockfile·`plugin.json`·`components.json`의 SemVer를 함께 올리고 같은 버전의 CHANGELOG 항목을 추가합니다. 공개 전 같은 버전의 후속 변경은 묶을 수 있지만 태그 공개 뒤 동일 버전 변경은 `make check`, PR/main CI, 후보·정식 게시 명령이 차단합니다.
