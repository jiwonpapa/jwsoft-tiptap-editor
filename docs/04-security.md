...
- 활성화 전에는 설정 화면을 열 수 없는 G7 계약에 맞춰 사이트 전체 `legacyContentRiskAcknowledged` 스위치와 선행 활성화 차단을 폐기했습니다. 기존 저장 키는 삭제하지 않으며 더 이상 사용하지 않습니다.
- 설치·활성화·조회만으로 기존 글의 저장된 본문을 변경하지 않습니다. 기존 글을 JWSoft에서 수정 후 저장할 때만 정제 결과가 저장됩니다.
- 기존 문서 정제 결과가 달라지면 해당 편집 화면을 읽기 전용으로 유지하고, 사용자가 위험을 확인해야 편집·저장이 가능합니다. 빈 새 글·호환 HTML·읽기 전용 조회에는 전환 경고를 띄우지 않습니다. 새 글의 붙여넣기 정제 안내와 서버 저장 검증은 별도로 유지합니다.
