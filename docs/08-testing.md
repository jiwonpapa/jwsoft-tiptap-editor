...
독립 Chromium 검사는 데스크톱 선택 영역 서식·키보드 포커스·링크·인용·목록·정렬·들여쓰기, mock API 이미지 업로드와 caption·정렬·크기 재편집 및 412px 반응형 출력, 화면 이동 100회의 mount/unmount instance 회수, Pixel 7 viewport의 툴바 스크롤·대화상자 폭을 검증합니다. 공개 `alpha.12`는 실제 인증 G7의 게시판·상품·페이지 작성/재편집, 한국어·영어 UI, 접근성 이름과 단일 instance, 412px 다크 테마를 screenshot·JSON으로 기록했습니다. 실제 한글 IME·Android/iOS 실기기는 release 단계에서 별도 증거를 생성합니다.
