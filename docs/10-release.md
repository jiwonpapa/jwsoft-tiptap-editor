...
버전을 올릴 때는 같은 커밋의 `CHANGELOG.md`에 날짜·수정 내용·보안 영향·제한을 기록합니다. 후보와 최종 GitHub 릴리스 본문은 해당 버전 항목을 자동 포함하며, 누락·중복·빈 변경 목록이면 게시하지 않습니다. 최종 승격 시에도 변경 목록과 후보 marker가 일치해야 하며 수동으로 다른 버전의 내역을 붙이지 않습니다.
